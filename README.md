...
在 **AstrBot 插件市场** 搜索并安装 `astrbot_plugin_live2d_adapter`，启用后：

1. 在平台适配器页添加一个 `live2d` 实例，配置连接与资源参数。
2. 在插件配置页配置独立表演规划 LLM（可选）。
3. 将 `auth_token` 填入桌面端连接设置。

## 能力概览

- `input.message` / `input.touch` / `input.shortcut` 双向事件桥接
- 消息链转换为 Live2D 表演序列（文字、动作、表情、TTS）
- `perform.show` 已支持 `interruptible`、表情 `combo` / `semantic`、`holdMs` / `resetPolicy`，并可按客户端能力退化为基础单表情模式
- `state.model` 已支持能力声明、表情目录、语义预设、模型发现摘要回传
...
5. 大资源通过同一服务入口下的 `/resources` 路径传输，默认与 WebSocket 共用 9090 端口
...
single_port_mode: true       # 默认同一端口同时提供 WebSocket 与资源接口
public_origin: ""           # 可选，公网/反代入口，如 https://example.com:9090

resource_enabled: true
resource_host: "127.0.0.1"  # 兼容旧双端口模式；single_port_mode=true 时自动复用 ws_host
resource_port: 9091          # 兼容旧双端口模式；single_port_mode=true 时自动复用 ws_port
resource_path: "/resources"
resource_dir: "live2d_resources"
temp_dir: "live2d_temp"
resource_base_url: ""       # 可选高级项；默认自动推导或使用 public_origin
resource_token: ""          # 留空复用 auth_token；单端口模式下默认如此
```

## 独立表演规划配置

独立表演规划现在属于**插件级配置**，不再属于平台实例配置。

- 配置入口：AstrBot 插件页 -> `Live2D 适配器` -> `planner`
- 推荐模式：`mode=provider`
- `provider_id`：使用 `_special: select_provider` 选择 AstrBot 已配置的独立对话 Provider
- 推荐单独创建一个低成本、稳定输出 JSON 的 Provider，避免干扰主对话模型

## 协议升级提示

- 如果桌面端已实现表情组合能力，请确保在 `state.model.capabilities.expressionCombo` 中显式声明，并为可组合条目设置 `expressionCatalog[].supportsCombo=true`。
- 如果桌面端已实现语义表情能力，请在 `state.model.capabilities.semanticExpression` 中显式声明，并同步回传 `semanticPresets`。
- 旧版仅支持 `expression.id` 的桌面端仍可兼容，但无法消费 `combo`、`semantic`、`holdMs`、`resetPolicy` 和 `interruptible` 带来的增强行为。
- 详细结构请查看：[`docs/API.md`](./docs/API.md)
...
1. 将 `ws_host` 调整为可监听地址（如 `0.0.0.0`），并保持 `single_port_mode: true`
2. 若桌面端通过公网 IP、域名或反向代理访问，请设置 `public_origin`
3. 云安全组仅放行 TCP `9090`
4. 主机防火墙同步仅放行相同端口
5. **务必设置来源 IP 白名单**
6. 使用强随机 token；公网场景推荐配合 WSS/反向代理，但并非强制
